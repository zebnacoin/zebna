...
#if defined(NDEBUG)
# error "Zebna cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Zebna Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ZEBNA additional fees, %d delta bytes\n",
