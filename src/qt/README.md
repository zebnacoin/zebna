This directory contains the ZebnaQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Zebna UI.
...
ClientModel is used by the main application `zebnagui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/zebnacoin/zebna/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a zebna:... link from a browser).
...
* UI elements like ZebnaAmountField, which inherit from QWidget.
* `zebnastrings.cpp`: automatically generated
* `zebnaunits.(h/cpp)`: ZEBNA / mZEBNA / etc handling
* `callback.h`
...
* don't change `local/zebna_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
4. Enter "zebna-qt" as project name, enter src/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "zebna-qt" under "Run", which is where you can also add command line arguments)
