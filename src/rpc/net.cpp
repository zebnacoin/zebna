...
static UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("addnode", "\"192.168.0.6:15555\" \"onetry\"")
            + HelpExampleRpc("addnode", "\"192.168.0.6:15555\", \"onetry\"")
        );
...
static UniValue disconnectnode(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() == 0 || request.params.size() >= 3)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("disconnectnode", "\"192.168.0.6:15555\"")
            + HelpExampleCli("disconnectnode", "\"\" 1")
            + HelpExampleRpc("disconnectnode", "\"192.168.0.6:15555\"")
...
static UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:15555\",  (string) The zebna server IP and port we're connected to\n"
