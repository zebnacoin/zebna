...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"          (string, required) The Zebna address (or hex-encoded script)\n"
            "2. \"label\"            (string, optional, default=\"\") An optional label\n"
            "3. rescan               (boolean, optional, default=true) Rescan the wallet for transactions\n"
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Zebna address or script");
        }
    }
    if (fRescan)
    {
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The zebna address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to zebnad)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/zebnacoin/zebna/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Zebna %s\n", CLIENT_BUILD);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see zebnad log for details) and could "
