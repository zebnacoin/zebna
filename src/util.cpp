...
const char * const ZEBNA_CONF_FILENAME = "zebna.conf";
const char * const ZEBNA_PID_FILENAME = "zebnad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "zebnad -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Zebna
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Zebna
    // Mac: ~/Library/Application Support/Zebna
    // Unix: ~/.zebna
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Zebna";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Zebna";
#else
    // Unix
    return pathRet / ".zebna";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ZEBNA_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", ZEBNA_PID_FILENAME)));
}
