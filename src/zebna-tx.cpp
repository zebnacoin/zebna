...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " zebna-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  zebna-tx [options] <hex-tx> [commands]  Update hex-encoded zebna transaction\n" +
            "or:     zebna-tx [options] -create [commands]   Create hex-encoded zebna transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded zebna transaction
