...
###   zebnacore.org and zebna.org and compares them.
...
WORKINGDIR="/tmp/zebna_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://zebna.com"
HOST2="https://zebna.com"
BASEDIR="/bin/"
VERSIONPREFIX="zebna-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'zebna-',
...
if [ $? -ne 0 ]; then
   echo "zebna.org failed to provide signature file, but zebnacore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "zebna.org and zebnacore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Zebna Core binary release signing key installed?"
   fi
